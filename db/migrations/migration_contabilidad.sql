-- ============================================================================
-- MAM ERP - Migracion: Modulo de Contabilidad y Tesoreria
-- Ejecutar sobre base de datos de produccion existente
-- Fecha: 2026-03-19
-- ============================================================================
...
-- 1.1 roles: agregar puc_code
ALTER TABLE `roles` ADD COLUMN IF NOT EXISTS `puc_code` VARCHAR(20) NULL COMMENT 'Codigo PUC asociado al rol';

-- 1.2 users: columnas para bot de ventas
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `bot_api_key` VARCHAR(255) NULL COMMENT 'API key del vendedor para bot';
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `bot_sheet_id` VARCHAR(255) NULL COMMENT 'ID del Google Sheet del vendedor';
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `bot_script_url` VARCHAR(500) NULL COMMENT 'URL del Google Apps Script';
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `bot_gid` VARCHAR(50) DEFAULT '0' COMMENT 'GID de la hoja dentro del Sheet';

-- 1.3 payments: origen del pago (caja/banco)
...
ALTER TABLE `payments` ADD INDEX IF NOT EXISTS `idx_origin` (`originType`, `originId`);
ALTER TABLE `payments` ADD INDEX IF NOT EXISTS `idx_cash_movement` (`cashMovementId`);

-- 1.4 entries: tienda, centro de costo, tipo de transaccion
...
ALTER TABLE `entries` ADD INDEX IF NOT EXISTS `idx_transaction` (`entryTransactionType`, `entryTransactionId`);
ALTER TABLE `entries` ADD INDEX IF NOT EXISTS `idx_store` (`entryStoreId`);
ALTER TABLE `entries` ADD INDEX IF NOT EXISTS `idx_date` (`entryDate`);
ALTER TABLE `entries` ADD INDEX IF NOT EXISTS `idx_cost_center` (`cost_center_id`);

-- 1.4b auxiliary_subaccounts: agregar accountType
ALTER TABLE `auxiliary_subaccounts` ADD COLUMN IF NOT EXISTS `accountType` VARCHAR(50) NULL COMMENT 'client, provider, employee, partner, other';

-- 1.5 subaccounts: PUC code, store, tipo de cuenta
...
ALTER TABLE `subaccounts` ADD INDEX IF NOT EXISTS `idx_puc_store` (`pucCode`, `store`);
ALTER TABLE `subaccounts` ADD INDEX IF NOT EXISTS `idx_account_type` (`accountType`);
ALTER TABLE `subaccounts` ADD INDEX IF NOT EXISTS `idx_store` (`store`);

-- 1.6 accounts_class: PUC code, store
...
ALTER TABLE `accounts_class` ADD INDEX IF NOT EXISTS `idx_puc_store` (`pucCode`, `store`);
ALTER TABLE `accounts_class` ADD INDEX IF NOT EXISTS `idx_puc` (`pucCode`);

-- 1.7 accounts_group: PUC code
ALTER TABLE `accounts_group` ADD COLUMN IF NOT EXISTS `pucCode` VARCHAR(20) NULL;
ALTER TABLE `accounts_group` ADD INDEX IF NOT EXISTS `idx_puc` (`pucCode`);

-- 1.8 accounts_accounts: PUC code
ALTER TABLE `accounts_accounts` ADD COLUMN IF NOT EXISTS `pucCode` VARCHAR(20) NULL;
ALTER TABLE `accounts_accounts` ADD INDEX IF NOT EXISTS `idx_puc` (`pucCode`);

-- ============================================================================
-- 2. ACTUALIZAR PUC CODES EN TABLAS EXISTENTES
-- ============================================================================

-- Poblar pucCode en accounts_class (1 digito)
UPDATE `accounts_class` SET `pucCode` = CAST(`classID` AS CHAR) WHERE `pucCode` IS NULL;

-- Poblar pucCode en accounts_group (2 digitos)
UPDATE `accounts_group` SET `pucCode` = CAST(`groupID` AS CHAR) WHERE `pucCode` IS NULL;

-- Poblar pucCode en accounts_accounts (4 digitos)
UPDATE `accounts_accounts` SET `pucCode` = CAST(`accountID` AS CHAR) WHERE `pucCode` IS NULL;

-- Poblar pucCode en subaccounts (6 digitos)
UPDATE `subaccounts` SET `pucCode` = CAST(`accountID` AS CHAR) WHERE `pucCode` IS NULL;

-- ============================================================================
-- 3. NUEVAS TABLAS - TESORERIA (Cajas y Bancos)
-- ============================================================================
...
-- ============================================================================
-- 4. NUEVAS TABLAS - CUENTAS POR PAGAR (Proveedores)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS `supplier_invoice_details` (
  ...
  CONSTRAINT `fk_sid_supplier_invoice` FOREIGN KEY (`supplierInvoiceId`)
    REFERENCES `supplier_invoices` (`idSupplierInvoice`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE IF NOT EXISTS `supplier_payments` (
  `idSupplierPayment` INT(11) NOT NULL AUTO_INCREMENT,
  `supplierInvoiceId` INT(11) NULL,
  `providerId` INT(11) NOT NULL,
...
-- ============================================================================
-- 5. NUEVAS TABLAS - GASTOS OPERACIONALES
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS `expense_categories` (
  ...
  `accounting_subaccount_id` INT(11) NULL,
...
('CAT004', 'Suministros de Oficina', 'Papeleria, tinta, utiles de oficina', 1, NOW()),
('CAT005', 'Mantenimiento y Reparaciones', 'Reparaciones de equipos, instalaciones', 1, NOW()),
('CAT006', 'Alimentacion', 'Alimentacion de empleados, refrigerios', 1, NOW()),
('CAT007', 'Aseo y Cafeteria', 'Productos de aseo, cafe, agua', 1, NOW()),
('CAT008', 'Publicidad y Marketing', 'Publicidad, volantes, redes sociales', 1, NOW()),
('CAT009', 'Honorarios Profesionales', 'Contador, abogado, consultoria', 1, NOW()),
('CAT010', 'Otros Gastos', 'Gastos varios no categorizados', 1, NOW());
...
-- ============================================================================
-- 6. NUEVAS TABLAS - PERIODOS CONTABLES
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS `accounting_periods` (
  ...
  `totalCosts` DECIMAL(15,2) DEFAULT 0,
...
-- ============================================================================
-- 7. NUEVAS TABLAS - CENTROS DE COSTO
-- ============================================================================
...
INSERT IGNORE INTO `cost_centers` (`code`, `name`, `type`, `store_id`, `is_active`, `created_at`) VALUES
('TDA-001', 'Tienda - Principal', 'tienda', 1, 1, NOW()),
('ADM-001', 'Administracion General', 'departamento', NULL, 1, NOW()),
('OPE-001', 'Operaciones', 'departamento', NULL, 1, NOW()),
('COM-001', 'Comercial / Ventas', 'departamento', NULL, 1, NOW());

-- ============================================================================
-- 8. NUEVAS TABLAS - CONFIGURACION CONTABLE
-- ============================================================================
...
-- Resolver subaccount_id desde puc_code
...
-- ============================================================================
-- 9. NUEVAS TABLAS - DEPARTAMENTOS Y KPIs
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS `departments` (
  ...
  `name` VARCHAR(100) NOT NULL,
  `description` TEXT NULL,
  `leader_user_id` VARCHAR(100) NULL,
  `store_id` INT(11) NULL,
...
INSERT IGNORE INTO `departments` (`name`, `description`, `active`, `created_at`) VALUES
('Ventas', 'Departamento comercial y ventas', 1, NOW()),
('Cartera', 'Gestion de cobros y cartera', 1, NOW()),
('Bodega', 'Almacenamiento y logistica', 1, NOW()),
('Administracion', 'Gestion administrativa y contable', 1, NOW());
...
-- ============================================================================
-- 10. NUEVAS TABLAS - PERMISOS Y BOT
-- ============================================================================
...
-- 1. Verificar que los pucCode se actualizaron correctamente en subaccounts
-- 2. Verificar que accounting_settings tiene los subaccount_id resueltos
-- 3. Crear al menos una caja: Ir a Tesoreria > Cajas > Nueva Caja
-- 4. Crear al menos una cuenta bancaria si aplica
-- ============================================================================
