                ...
                <div class="px-6 mx-auto grid">
                    ...
                    <form action="<?php echo base_url(); ?>sisvent/admin/expenses/update" method="POST">
                        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md">
                            ...
                            <?php
                                $isBanco = ($expense->payment_method == 'transferencia' || $expense->source_type == 'banco');
                            ?>
                            <div class="grid grid-cols-2 gap-4 mt-4">
                                ...
                                <div>
                                    ...
                                    <div id="caja-select-wrapper" <?php echo $isBanco ? 'style="display:none;"' : ''; ?>>
                                        <select class="form-input form-select" name="source_id_caja" id="source_id_caja">
                                            ...
                                            <?php foreach($cashboxes as $cb): ?>
                                                <option value="<?php echo $cb->idCashbox; ?>"
                                                        <?php echo (!$isBanco && $expense->source_id == $cb->idCashbox) ? 'selected' : ''; ?>>
                                                    <?php echo $cb->name; ?> ($<?php echo number_format($cb->currentBalance, 2); ?>)
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div id="banco-select-wrapper" <?php echo !$isBanco ? 'style="display:none;"' : ''; ?>>
                                        <select class="form-input form-select" name="source_id_banco" id="source_id_banco">
                                            <option value="">Seleccione...</option>
                                            <?php foreach($bankaccounts as $ba): ?>
                                                <option value="<?php echo $ba->idBankAccount; ?>"
                                                        <?php echo ($isBanco && $expense->source_id == $ba->idBankAccount) ? 'selected' : ''; ?>>
                            ...
                            <div id="payment-section" class="mt-4" <?php echo ($expense->status != 'pagado') ? 'style="display:none;"' : ''; ?>>
                                <label class="block text-sm">
                            ...
                            <!-- Campos ocultos -->
                            <input type="hidden" name="source_type" id="source_type" value="<?php echo $isBanco ? 'banco' : 'caja'; ?>"/>
