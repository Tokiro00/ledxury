        ...
        <div class="flex flex-col flex-1 w-full">
            ...
            <main class="h-full overflow-y-auto">
                <div class="px-6 mx-auto grid max-w-4xl">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-700"><?php echo $bill->concept ?: 'Sin descripción'; ?></p>
                                    </div>
                                </div>
                            ...
                            <div class="mt-6 pt-6 border-t border-gray-200">
                                <div class="grid grid-cols-3 gap-4 text-center">
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        ...
                                        <p class="text-2xl font-bold text-gray-800">$<?php echo number_format($bill->total, 2); ?></p>
                                    </div>
                                    <div class="bg-green-50 rounded-lg p-4">
                                        <p class="text-xs text-green-600 uppercase">Total Pagado</p>
                                        <p class="text-2xl font-bold text-green-700">$<?php echo number_format($bill->paidAmount, 2); ?></p>
                                    </div>
                                    <div class="bg-red-50 rounded-lg p-4">
                                        <p class="text-xs text-red-600 uppercase">Saldo Pendiente</p>
                                        <p class="text-2xl font-bold text-red-700">$<?php echo number_format($bill->balance, 2); ?></p>
                                    </div>
                                </div>
                            ...
                            <tbody class="divide-y">
                                ...
                                <tr class="text-gray-700">
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <?php echo $payment->paymentMethod == 'cashbox' ? 'Efectivo (Caja)' : 'Transferencia (Banco)'; ?>
                                    </td>
                                    <td class="px-6 py-4 text-sm"><?php echo $payment->reference ?: '-'; ?></td>
                                    <td class="px-6 py-4 text-sm text-right font-semibold text-green-600">$<?php echo number_format($payment->amount, 2); ?></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                            <tfoot>
                                <tr class="bg-gray-50 font-semibold">
                                    ...
                                    <td class="px-6 py-3 text-right text-green-700">$<?php echo number_format($bill->paidAmount, 2); ?></td>
                                </tr>
                            </tfoot>
                        </table>
                        <?php else: ?>
                    ...
                    <!-- Actions -->
                    <?php if($bill->status != 'anulada' && $bill->paidAmount == 0): ?>
                    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                        <h4 class="text-sm font-semibold text-gray-700 mb-4">Acciones</h4>
                        <form action="<?php echo base_url(); ?>sisvent/admin/accountspayable/cancel/<?php echo $bill->idSupplierInvoice; ?>" method="POST" onsubmit="return confirm('¿Está seguro que desea anular esta factura? Esta acción no se puede deshacer.');">
                            <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700">
                                Anular Factura
                            </button>
                            <p class="text-xs text-gray-500 mt-2">Solo se pueden anular facturas sin pagos registrados.</p>
                        </form>
                    </div>
                    <?php endif; ?>
