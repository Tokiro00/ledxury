...
class Message_model extends CI_model{
	...
	*/
	public function logoutUser($status, $date){
		if(isset($this->session->userdata('user_data')['uname'])){
			$currentSession = $this->session->userdata('user_data')['uname'];
			$this->db->query("UPDATE users SET user_status = '$status' , last_logout = '$date' WHERE 
			idUser = '$currentSession'");
		}
	}
	...
	public function getmessage($data){
		$this->db->select('*');
		$session_id = $this->session->userdata('user_data')['uname'];
	...
	public function getUnreadMessagesCount($id, $sender = ''){
		$this->db->select('*');
		$this->db->from('user_messages');
		$this->db->where('receiver_message_id',$id);
		if(!empty($sender))
        {
        	$this->db->where('sender_message_id', $sender);
        }
	...
	public function clearMessages($id, $sender){

		$data = array('readed' => 1);
		$this->db->where('receiver_message_id',$id);
        $this->db->where('sender_message_id', $sender);

		return $this->db->update("user_messages",$data);		
	}
