$(document).ready(function () {
	...
	MAIN_PLAY.from("#main", { duration: 0.5, opacity: 1 });
	...
	function getUserList() {
		return new Promise(function (resolve, reject) { //Creating new Promise Chain
			...
			$(document).on("mouseleave", '.innerBox', function () {
			//$('.innerBox').mouseleave(function () {
				inter2 = setInterval(getUserList, 1000);
			});*/
			if(unique_id){
				sendUserUniqIDForMsg(unique_id, bg_image, false);
			}
		})
	}

	$(document).on("click", '.innerBox', function () {
		...
		sendUserUniqIDForMsg(unique_id, bg_image, true);
	...
	function setUserDetails(data) {
		...
		var avtar = `../public/dist/images/${data[0]['picture_url']}`;
	...
	function sendUserUniqIDForMsg(uniq_id, bg_image, clear_msgs) {
		$.post('Message/getmessage', { data: uniq_id, image: bg_image, clear: clear_msgs }, function (data) {
	...
	$(document).on("keydown", '#messageText', function(event){
	...
	$(document).on("click", '#update_container i', function () {
